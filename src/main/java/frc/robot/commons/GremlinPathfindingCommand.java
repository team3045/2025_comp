...
public class GremlinPathfindingCommand extends Command {
   ...
   */
  public GremlinPathfindingCommand(
    ...
    Rotation2d finalTargetRotation = targetRotation;

    this.targetPath = targetPath;
    this.targetPose = () -> new Pose2d(this.targetPath.getPoint(0).position, finalTargetRotation);
    this.originalTargetPose = new Pose2d(this.targetPose.get().getTranslation(), this.targetPose.get().getRotation());
  ...
  public GremlinPathfindingCommand(
    ...
    this.targetPose = targetPose;
    this.originalTargetPose = new Pose2d(this.targetPose.get().getTranslation(), this.targetPose.get().getRotation());
    this.goalEndState = new GoalEndState(goalEndVel.getAsDouble(), targetPose.get().getRotation());
    this.constraints = constraints;
    this.controller = controller;
    this.poseSupplier = poseSupplier;
    this.speedsSupplier = speedsSupplier;
  ...
  public void initialize() {
    ...
    if (targetPath != null) {
      ...
      if (shouldFlipPath.getAsBoolean()) {
        ...
        goalEndState = new GoalEndState(goalEndState.velocityMPS(), targetPose.get().getRotation());
      }
    }

    if (currentPose.getTranslation().getDistance(targetPose.get().getTranslation()) < 0.5) {
  ...
  public void execute() {
    ...
    boolean skipUpdates = currentTrajectory != null
        && currentPose
            .getTranslation()
            .getDistance(currentTrajectory.getEndState().pose.getTranslation()) < tooCloseSkipDistance;
  ...
  public boolean isFinished() {
    ...
    if (targetPath != null && !targetPath.isChoreoPath()) {
      ...
      return currentPose.getTranslation().getDistance(targetPose.get().getTranslation()) <= stoppingDistance;
    }
  ...
  public static Command warmupCommand() {
    return new GremlinPathfindingCommand(
