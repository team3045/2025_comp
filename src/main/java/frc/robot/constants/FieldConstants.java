...
public class FieldConstants {
        ...
        public static final double edgeToTag = 0.0508 + (aprilTagWidth / 2); // 2 inches to edge and half of width to
                                                                             // center
        ...
        public static final double reefDistanceTolerance = 2.5;
        public static final List<AprilTag> shopTags = List.of(
                        ...
                        new AprilTag(10, new Pose3d(
                                        ...
                                        0.22225,
                                        new Rotation3d(0, 0, -Math.PI / 2))));
        // Stuff to transform shop field apriltags relative to apriltag 10 on our mock
        // reef
        ...
        static {
                StructArrayPublisher<Pose3d> pub = NetworkTableInstance.getDefault()
                                .getStructArrayTopic("ShopTags", Pose3d.struct).publish();
                for (AprilTag tag : shopTags) {
                        Pose3d tagPose = tag.pose;
                        Pose3d transformedPose = tagPose.rotateBy(transformToRealField.getRotation());
                        Translation3d transform = fieldTag10Pose.getTranslation().minus(
                                        shopTag10Pose.rotateBy(transformToRealField.getRotation()).getTranslation());
                        transformedPose = new Pose3d(transformedPose.getTranslation().plus(transform),
                                        transformedPose.getRotation());
